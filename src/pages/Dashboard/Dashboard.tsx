...
import { ActionButton, DeleteButton, Button } from '../../components/Dashboard/DashboardButton';
...
export const Dashboard: React.FC = () => {
  ...
  const userAvatar = eventos.find((e: Evento) => e.id === '4')?.imagen || '';
      ...
      <div className={styles.mobileLayout}>
        <div className={styles.mobileLayout__profile}>
          <div className={styles.mobileLayout__profileHeader}>
            <img src={userAvatar} alt={currentName} className={styles.mobileLayout__avatar} />
            <div className={styles.mobileLayout__profileInfo}>
              <h4>{currentName}</h4>
              <span>Modo Organizador</span>
            </div>
          </div>
        ...
        <section className={styles.mobileLayout__statsGrid}>
          <div className={`baseCard ${styles.dashboardStatCard}`}>
            ...
            <div className={styles.dashboardStatCard__value}>{stats.eventosActivos}</div>
          </div>
          ...
          <div className={`baseCard ${styles.dashboardStatCard}`}>
            ...
            <div className={styles.dashboardStatCard__value}>{stats.asistentesTotales}</div>
          </div>
        ...
        <div className={styles.mobileLayout__searchBar}>
          <Search size={16} />
          <input type="text" placeholder="Buscar eventos..." value={searchTerm} onChange={handleSearch} />
        </div>
        
        <div className={styles.mobileLayout__filterTabs}>
          <button type="button" className={activeFilter === 'todos' ? styles.tabActive : ''} onClick={() => setActiveFilter('todos')}>Todos los eventos</button>
          <button type="button" className={activeFilter === 'proximos' ? styles.tabActive : ''} onClick={() => setActiveFilter('proximos')}>Próximos</button>
        </div>
      </div>

        <section className={styles.mobileLayout__eventsSection}>
          {eventos?.filter((e: Evento) => e.id !== '4').map((evento: Evento) => (
            <div key={evento.id} className={`baseCard ${styles.eventCardMobile}`}>
              <div className={styles.eventCardMobile__imageBox}>
                <img src={evento.imagen} alt={evento.titulo} />
                <span className={styles.eventCardMobile__badge}>Activo</span>
              </div>
              <h3>{evento.titulo}</h3>
              <p className={styles.eventCardMobile__meta}><Calendar size={12} /> {evento.fecha}</p>
              <p className={styles.eventCardMobile__meta}>📍 {evento.ubicacion}</p>
              <p className={styles.eventCardMobile__meta}>👥 {evento.vistas} vistas</p>
                        <div className={styles.eventCardMobile__actions}>
                <ActionButton onClick={() => navigate(`/edit-event/${evento.id}`)}>
                  Editar
                </ActionButton>
      ...
      <div className={styles.desktopLayout}>
        <header className={styles.desktopLayout__header}>
          <div className={styles.desktopLayout__headerInfo}>
            ...
            <p className="subTitle">Gestione sus eventos, monitoree la asistencia.</p>
          </div>
        ...
        <section className={styles.desktopLayout__filterSection}>
          <div className={styles.desktopLayout__titleRow}>
            <h2 className={styles.desktopLayout__mainSectionTitle}>Eventos Creados</h2>
          </div>
        ...
        <section className={styles.desktopLayout__mainWrapper}>
          {(() => {
            ...
            return (
              <div key={String(evento.id)} className={`baseCard ${styles.eventCardDesktop}`}>
